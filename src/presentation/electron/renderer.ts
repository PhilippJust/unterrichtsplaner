...
document
  ...
  ?.addEventListener('submit', (evt: Event) => {
    ...
    const anfrage: UnterrichtsablaufAnfrage = {
      ...
      klassengroesse: parseInt(
        (document.getElementById('klassengroesse') as HTMLInputElement).value
      ),
...
function populateTable(tableId: string, data: Aktion[]) {
  const tableBody = (
  ...
  for (const aktion of data) {
    ...
    row.insertCell(0).innerText = aktion.dauer.toString()
